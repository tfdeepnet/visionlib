...
class LRFinder(object):
    ...
    def range_test(
        ...
        self.history_accy = {"lr": [], "loss": [], "accuracy": []}
        ...
        for iteration in tqdm(range(num_iter)):
            ...
            self.history_accy["lr"].append(lr_schedule.get_lr()[0])
    ...
    def plot_accuracy(
        ...
        # Get the data to plot from the history dictionary. Also, handle skip_end=0
        # properly so the behaviour is the expected
        lrs = self.history_accy["lr"]
        losses = self.history_accy["accuracy"]
        if skip_end == 0:
            lrs = lrs[skip_start:]
            losses = losses[skip_start:]
        else:
        ...
        if suggest_lr:
            ...
            min_grad_idx = None
            try:
                min_grad_idx = (np.gradient(np.array(losses))).argmax()
            except ValueError:
                print(
                    "Failed to compute the gradients, there might not be enough points."
                )
